...
def execute(qc, backend='qasm_simulator', shots=1024):
    """
    Execute a quantum circuit on the specified backend.

    Parameters:
        qc (QuantumCircuit): The quantum circuit to execute.
        backend (str or BaseBackend): The backend to execute the circuit on. Default is 'qasm_simulator'.
        shots (int): The number of shots (measurements) to perform. Default is 1024.

    Returns:
        job (BaseJob): The job object representing the execution of the circuit.
    """
    if isinstance(backend, str):
        backend = AerSimulator(method=backend)
